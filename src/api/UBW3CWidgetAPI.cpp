...
// WistOpenboard fork: hand the URL to the OS default browser directly. This
// bypasses OpenBoard's internal web mode entirely. Required for sites that
// reject framing (e.g. YouTube /watch with X-Frame-Options: SAMEORIGIN) AND
// where the internal browser also fails — only the user's real browser, with
// matching cookies and a trusted origin, is guaranteed to work.
void UBW3CWidgetAPI::openExternalURL(const QString& url)
{
