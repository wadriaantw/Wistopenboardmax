...
void UBMetadataDcSubsetAdaptor::persist(std::shared_ptr<UBDocumentProxy> proxy)
{
    ...
    // WistOpenboard fork: remember last visited page across sessions
    xmlWriter.writeTextElement(UBSettings::uniboardDocumentNamespaceUri, "last-visited-page",
                               QString::number(proxy->lastVisitedSceneIndex()));
    ...
    if (file.exists())
    {
        ...
        while (!xml.atEnd())
        {
            ...
            if (xml.isStartElement())
            {
                ...
                else if (name == "last-visited-page" // WistOpenboard fork
                        && xml.namespaceUri() == UBSettings::uniboardDocumentNamespaceUri)
                {
                    metadata.insert(UBSettings::documentLastVisitedPage, xml.readElementText());
                }
