...
void UBDesktopAnnotationController::updateBackground()
{
    ...
    {
        // Capture mode: window must be truly invisible (no overlay tint in screenshot)
    ...
    {
        // WistOpenboard fork: pass-through behavior for Selector mode in Desktop
        // overlay. With Qt::transparent (alpha=0) Windows lets mouse events pass
        // straight through to the underlying desktop, so the user can click and
        // drag actual desktop items / app windows. For drawing tools we keep
...
#else
            newBrush = QBrush(QColor(127, 127, 127, 4));
#endif
        }
#ifdef UB_REQUIRES_MASK_UPDATE
        updateMask(false);
