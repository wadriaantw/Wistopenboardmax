...
{
    // WistOpenboard fork: prefer persisted metadata value if available
    if (mMetaDatas.contains(UBSettings::documentLastVisitedPage))
    {
        bool ok = false;
        int idx = mMetaDatas.value(UBSettings::documentLastVisitedPage).toInt(&ok);
        if (ok && idx >= 0)
            return idx;
    }
...
void UBDocumentProxy::setLastVisitedSceneIndex(int lastVisitedSceneIndex)
{
    ...
    // WistOpenboard fork: also store in metadata so it persists to disk via the metadata adaptor
    setMetaData(UBSettings::documentLastVisitedPage, QString::number(lastVisitedSceneIndex));
}
