...
signals:
    ...
    private:
        ...
        // Adaptive eraser state — radius scales with stroke speed and pressure.
        QElapsedTimer mEraserClock;
        QList<QPair<QPointF, qint64>> mEraserSamples;  // recent (pos, t) — windowed average
        qreal mEraserSmoothedRadius = 0.0;
        qreal mEraserCurrentRadius = 0.0;              // last computed adaptive radius
        qreal computeAdaptiveEraserRadius(const QPointF& pos, qreal pressure);
        ...
        // WistOpenboard fork: snap-to-shape — raw scene-coord points captured during a Pen/Marker stroke,
        // used by trySnapStrokeToShape() in inputDeviceRelease.
        QVector<QPointF> mShapePoints;
        // Replace the polygons making up the current stroke with clean primitive geometry
        // (line / circle / rectangle / square). Returns true if a replacement was made.
        // Must be called BEFORE the stroke is wrapped in a UBGraphicsStrokesGroup.
        bool trySnapStrokeToShape();
