...
#include "frameworks/UBShapeRecognizer.h" // WistOpenboard fork: snap-to-shape
...
bool UBGraphicsScene::inputDevicePress(const QPointF& scenePos, const qreal& pressure, Qt::KeyboardModifiers modifiers)
{
    ...
    if (mInputDeviceIsPressed) {
        ...
        if (UBDrawingController::drawingController()->isDrawingTool()) {
            ...
            // WistOpenboard fork: start capturing raw points for shape recognition.
            // Always capture (cheap) so toggling snap-to-shape mid-stroke still works on release.
            mShapePoints.clear();
            if (currentTool == UBStylusTool::Pen || currentTool == UBStylusTool::Marker) {
                mShapePoints.append(scenePos);
                qDebug() << "[snap-to-shape] press: capture started";
            }
            ...
            // Reset speed history at stroke start so radius doesn't carry over.
            mEraserSamples.clear();
            mEraserSmoothedRadius = 0.0;
            mEraserClock.invalidate();
            qreal eraserRadius = computeAdaptiveEraserRadius(scenePos, pressure);
            qreal eraserWidth = eraserRadius * 2.0;
...
bool UBGraphicsScene::inputDeviceMove(const QPointF& scenePos, const qreal& pressure, Qt::KeyboardModifiers modifiers)
{
    ...
    {
        // Update adaptive radius on hover so the preview circle scales as the
        // user moves, even before pressing.
        computeAdaptiveEraserRadius(position, mInputDeviceIsPressed ? 1.0 : 0.0);
    ...
    if (mInputDeviceIsPressed)
    {
        if (dc->isDrawingTool())
        {
            ...
            if(dc->activeRuler()){
                ...
                // WistOpenboard fork: snap-to-shape — capture raw move points (Pen/Marker only).
                if (currentTool == UBStylusTool::Pen || currentTool == UBStylusTool::Marker)
                    mShapePoints.append(scenePos);
        ...
        {
            qreal eraserRadius = computeAdaptiveEraserRadius(position, pressure);
            eraseLineTo(position, eraserRadius * 2.0);
        }
        else if (currentTool == UBStylusTool::Pointer)
        {
...
bool UBGraphicsScene::inputDeviceRelease(int tool, Qt::KeyboardModifiers modifiers)
{
    ...
    // Reset adaptive eraser state on release so the next interaction starts
    // small instead of carrying over the smoothed radius from a fast stroke.
    mEraserSamples.clear();
    mEraserSmoothedRadius = 0.0;
    mEraserCurrentRadius = 0.0;
    mEraserClock.invalidate();

    // Hide the eraser preview circle on release. On touch input the leaveEvent
    // does not always fire when the finger lifts, so the dashed preview circle
    // could stay on screen and (with cosmetic pen invalidation gaps) leave
    // visible arc fragments behind.
    if (mEraser && mEraser->isVisible()) {
        QRectF oldSceneRect = mEraser->sceneBoundingRect().adjusted(-2, -2, 2, 2);
        mEraser->hide();
        if (!oldSceneRect.isEmpty())
            invalidate(oldSceneRect, QGraphicsScene::ItemLayer);
    }
    ...
    if (dc->isDrawingTool(tool) || mDrawWithCompass)
    {
        if(mArcPolygonItem){
            ...
            // WistOpenboard fork: try to snap the just-finished stroke to a recognized primitive
            // (line / circle / rectangle / square). If a match is found, mCurrentStroke's polygons
            // are replaced in place; the StrokesGroup wrapping below picks up the new geometry.
            trySnapStrokeToShape();
...
qreal UBGraphicsScene::computeAdaptiveEraserRadius(const QPointF& pos, qreal pressure)
{
    // Tunables — idle small, fast big, but stable in between.
    qreal base = UBSettings::settings()->currentEraserWidth();
    base /= UBApplication::boardController->systemScaleFactor();
    base /= UBApplication::boardController->currentZoom();

    const qreal IDLE_FRACTION = 0.20;    // tiny idle dot
    const qreal MAX_GROWTH    = 20.0;    // up to 20× idle (≈ 2× the configured eraser)
    const qint64 WINDOW_MS    = 120;     // average speed over ~120 ms
    const qreal SMOOTHING     = 0.65;    // 65% prior, 35% target — reacts in ~3 frames

    qreal idleRadius = (base / 2.0) * IDLE_FRACTION;

    if (!mEraserClock.isValid()) mEraserClock.start();
    qint64 nowMs = mEraserClock.elapsed();

    // If there's a noticeable gap since the last event (user paused, switched
    // tools, etc.) treat this as a fresh start so the radius doesn't reuse a
    // stale smoothed value from a previous fast stroke.
    if (!mEraserSamples.isEmpty()) {
        qint64 lastT = mEraserSamples.last().second;
        if (nowMs - lastT > 250 || nowMs < lastT) {
            mEraserSamples.clear();
            mEraserSmoothedRadius = 0.0;
        }
    }

    // Append the new (pos, t) sample and drop ones outside the window.
    mEraserSamples.append({pos, nowMs});
    while (!mEraserSamples.isEmpty() && (nowMs - mEraserSamples.first().second) > WINDOW_MS)
        mEraserSamples.removeFirst();

    // Average speed = total path length / total time across the window.
    qreal totalDist = 0.0;
    qint64 totalDt  = 0;
    for (int i = 1; i < mEraserSamples.size(); ++i) {
        totalDist += QLineF(mEraserSamples[i-1].first, mEraserSamples[i].first).length();
        totalDt   += mEraserSamples[i].second - mEraserSamples[i-1].second;
    }
    qreal avgSpeed = (totalDt > 0) ? (totalDist / totalDt) : 0.0;  // scene-units / ms

    // Speed → multiplier curve. Square-root shape: steep rise on the first
    // bit of movement, then continues to grow toward MAX_GROWTH at high speeds.
    //   speed 0    → 1   (idle, small)
    //   speed 0.25 → ~5.5 (light drag — already visibly bigger)
    //   speed 1    → 10
    //   speed 4    → 19
    //   speed 9+   → 28-30 (capped)
    // Linear curve — small movements give small growth, sustained fast strokes
    // approach the cap. Avoid sqrt: it amplifies tiny taps into huge radii.
    //   speed 0    → 1    (idle)
    //   speed 0.5  → 2.5
    //   speed 1    → 4
    //   speed 3    → 10
    //   speed 6+   → 20   (capped)
    qreal speedFactor = 1.0 + qMin<qreal>(MAX_GROWTH - 1.0, avgSpeed * 3.0);

    // Pressure factor: 0 (no info) → 1, full pressure 1.0 → 2.5.
    // Modest pressure influence — full press only adds 50% on top of speed.
    qreal pressureFactor = (pressure > 0.0) ? (0.85 + 0.5 * pressure) : 1.0;

    qreal target = idleRadius * speedFactor * pressureFactor;
    target = qBound(idleRadius, target, idleRadius * MAX_GROWTH);

    // Exponential smoothing on the *radius* so it grows/shrinks gradually
    // instead of jumping each frame. Combined with the windowed-average
    // speed above, this gives a smooth, predictable response.
    if (mEraserSmoothedRadius <= 0.0) mEraserSmoothedRadius = target;
    mEraserSmoothedRadius = SMOOTHING * mEraserSmoothedRadius + (1.0 - SMOOTHING) * target;

    mEraserCurrentRadius = mEraserSmoothedRadius;
    return mEraserCurrentRadius;
}

void UBGraphicsScene::drawEraser(const QPointF &pPoint, bool pressed)
{
    if (mEraser) {
        // Use the radius last computed by inputDeviceDown/Move so we don't
        // double-step the adaptive smoothing each frame.
        qreal eraserRadius = (mEraserCurrentRadius > 0.0) ? mEraserCurrentRadius
                                                          : computeAdaptiveEraserRadius(pPoint, pressed ? 1.0 : 0.0);
        qreal eraserWidth = eraserRadius * 2.0;

        // The eraser preview uses a cosmetic dashed pen. Cosmetic pens paint
        // outside the item's bounding rect, so moving the rect can leave
        // dashed trails at the old position. Invalidate the old paint region
        // (with a small margin for the cosmetic pen width) before moving.
        QRectF oldSceneRect = mEraser->sceneBoundingRect().adjusted(-2, -2, 2, 2);
        if (!oldSceneRect.isEmpty()) {
            invalidate(oldSceneRect, QGraphicsScene::ItemLayer);
        }
...
// =====================================================================
// WistOpenboard fork: snap-to-shape
// =====================================================================
//
// Called from inputDeviceRelease, AFTER the stroke has been built and
// (optionally) simplified, but BEFORE the polygons are wrapped in a
// UBGraphicsStrokesGroup. If the captured raw points (mShapePoints)
// classify as a Line / Circle / Rectangle / Square and the user has
// the snap-to-shape setting enabled, the original freehand polygons
// are removed from the scene and replaced with clean straight-line
// polygon segments approximating the recognized primitive.
//
// Returns true if a replacement was made (caller can then assume
// mCurrentStroke->polygons() now contains the snapped geometry).
bool UBGraphicsScene::trySnapStrokeToShape()
{
    const bool snapEnabled = UBSettings::settings()->boardSnapToShape->get().toBool();
    qDebug() << "[snap-to-shape] release: enabled=" << snapEnabled
             << " mCurrentStroke=" << (mCurrentStroke != nullptr)
             << " points=" << mShapePoints.size();
    if (!snapEnabled)
        return false;
    if (!mCurrentStroke)
        return false;

    const UBStylusTool::Enum tool =
        (UBStylusTool::Enum)UBDrawingController::drawingController()->stylusTool();
    if (tool != UBStylusTool::Pen && tool != UBStylusTool::Marker) {
        qDebug() << "[snap-to-shape] tool not Pen/Marker (was" << (int)tool << ") — skipping";
        return false;
    }
    if (mShapePoints.size() < 6) {
        qDebug() << "[snap-to-shape] too few points";
        return false;
    }

    UBShapeRecognizer::Result r = UBShapeRecognizer::recognize(mShapePoints);
    qDebug() << "[snap-to-shape] recognized kind=" << (int)r.kind;
    if (r.kind == UBShapeRecognizer::None)
        return false;
    ...
    //
    // Tricky: each UBGraphicsPolygonItem's destructor calls clearStroke(),
    // which itself deletes the stroke when its polygons() list becomes empty.
    // If we deleted polys while leaving mCurrentStroke pointed at the same
    // stroke, the FIRST poly destructor would delete mCurrentStroke and the
    // second poly would dereference a dangling mStroke pointer (use-after-free
    // hang/crash — this is what was hanging the app on snap).
    //
    // Fix: swap in a fresh mCurrentStroke BEFORE deleting old polys. The old
    // polys still reference the old stroke; the last poly's destructor will
    // cleanly delete the (now-orphan) old stroke. mCurrentStroke is the new
    // stroke that the addPolygonItemToCurrentStroke calls below will populate.
    UBGraphicsStroke* oldStroke = mCurrentStroke;
    const QList<UBGraphicsPolygonItem*> oldPolys = oldStroke->polygons();
    mCurrentStroke = new UBGraphicsStroke(shared_from_this());

    foreach (UBGraphicsPolygonItem* poly, oldPolys) {
        mPreviousPolygonItems.removeAll(poly);
        mAddedItems.remove(poly);
        QGraphicsItem* gi = poly;
        if (gi->scene())
            removeItem(poly);
        UBCoreGraphicsScene::removeItemFromDeletion(poly);
        delete poly; // poly's dtor removes self from oldStroke; last one frees oldStroke
    }
    // oldStroke is now deleted by the last poly's destructor.

    auto emitSegment = [this, width](const QPointF& a, const QPointF& b) {
        if (QLineF(a, b).length() < 1e-6)
            return;
        UBGraphicsPolygonItem* p = lineToPolygonItem(QLineF(a, b), width, width);
        addPolygonItemToCurrentStroke(p);
    };

    switch (r.kind) {
    case UBShapeRecognizer::Line:
        emitSegment(r.p1, r.p2);
        break;
    case UBShapeRecognizer::Circle: {
        const int N = 64;
        constexpr qreal kTwoPi = 6.28318530717958647692; // avoid M_PI dependency on MSVC
        QPointF prev;
        for (int i = 0; i <= N; ++i) {
            const qreal t = (kTwoPi * i) / N;
            const QPointF p(r.center.x() + r.radius * std::cos(t),
                            r.center.y() + r.radius * std::sin(t));
            if (i > 0)
                emitSegment(prev, p);
            prev = p;
        }
    ...
    case UBShapeRecognizer::Rectangle:
    case UBShapeRecognizer::Square: {
        const QPointF tl = r.rect.topLeft();
        const QPointF tr = r.rect.topRight();
        const QPointF br = r.rect.bottomRight();
        const QPointF bl = r.rect.bottomLeft();
        emitSegment(tl, tr);
        emitSegment(tr, br);
        emitSegment(br, bl);
        emitSegment(bl, tl);
    ...
    case UBShapeRecognizer::Triangle: {
        emitSegment(r.triangle[0], r.triangle[1]);
        emitSegment(r.triangle[1], r.triangle[2]);
        emitSegment(r.triangle[2], r.triangle[0]);
        break;
    }
    case UBShapeRecognizer::Parabola: {
        // Sample y = a*x^2 + b*x + c across [xMin, xMax]; emit polyline segments.
        const int N = 48;
        const qreal xMin = r.parabolaXMin;
        const qreal xMax = r.parabolaXMax;
        QPointF prev;
        for (int i = 0; i <= N; ++i) {
            const qreal t = (qreal)i / N;
            const qreal x = xMin + (xMax - xMin) * t;
            const qreal y = r.parabolaA * x * x + r.parabolaB * x + r.parabolaC;
            QPointF p(x, y);
            if (i > 0) emitSegment(prev, p);
            prev = p;
        }
        break;
    }
...
void UBGraphicsScene::updateEraserColor()
{
    ...
    // No fill — just a thin dashed outline circle that follows the cursor.
    mEraser->setBrush(Qt::NoBrush);
    QPen p(mDarkBackground ? QColor(255, 255, 255, 180) : QColor(0, 0, 0, 180));
    p.setWidthF(1.5);
    p.setStyle(Qt::DashLine);
    p.setCosmetic(true); // constant on-screen width regardless of zoom
    mEraser->setPen(p);
}
