...
void UBGraphicsItemDelegate::createControls()
{
    ...
    // WistOpenboard fork: floating delete chip — sits at the top-left of any
    // selected item. Parent is nullptr (gets added to the scene on demand).
    if (!mFloatingDeleteButton) {
        mFloatingDeleteButton = new DelegateButton(":/images/close.svg", mDelegated, nullptr, Qt::TopLeftSection);
        mFloatingDeleteButton->setToolTip(tr("Delete"));
        mFloatingDeleteButton->setData(UBGraphicsItemData::ItemLayerType, QVariant(UBItemLayerType::Control));
        mFloatingDeleteButton->setZValue(std::numeric_limits<qreal>::max());
        mFloatingDeleteButton->hide();
        connect(mFloatingDeleteButton, SIGNAL(clicked()), this, SLOT(remove()));
    }
...
{
    Q_UNUSED(event)
    // WistOpenboard fork: show thin drag-border on hover, even before click.
    if (!mHovered) {
        mHovered = true;
        positionHandles();
    }
...
void UBGraphicsItemDelegate::positionHandles()
{
    ...
    // Show the minimal drag-border on hover too, not only on click-selection,
    // so the user knows where to grab the item before they've selected it.
    const bool active = mDelegated->isSelected() || mHovered;
    if (active) {
        bool shownOnDisplay = mDelegated->data(UBGraphicsItemData::ItemLayerType).toInt() != UBItemLayerType::Control;
        showHide(shownOnDisplay);
        ...
        // WistOpenboard fork: per user preference, hide the selection
        // delegate frame and its handle buttons. Items remain selectable and
        // movable (via Qt's standard ItemIsMovable drag handling), but no
        // visible box / X / arrows / resize handles surround the object.
        // To restore the original behavior, flip this to true.
        constexpr bool showFrame = false;

        // Special case: text items (GF_TITLE_BAR_USED) need their TitleBarArea
        // formatting buttons (font / color / +size / -size / align) visible.
        // Show the frame so those buttons can render (they're parented to it),
        // but suppress the frame body fill and resize handles so we don't
        // re-introduce the box-around-item the user dislikes.
        const bool isTextItem = testUBFlags(GF_TITLE_BAR_USED);
        const bool playMode = UBStylusTool::Play == UBDrawingController::drawingController()->stylusTool();

        if (isTextItem && !playMode) {
            mFrame->show();
            mFrame->setBrush(Qt::NoBrush);     // no visible body fill
            mFrame->setResizeHandlesVisible(false);
            updateButtons(true);
            // After updateButtons positioned everything, hide the non-title-bar
            // buttons so only the formatting strip is visible.
            foreach (DelegateButton *btn, mButtons) {
                if (btn->getSection() != Qt::TitleBarArea)
                    btn->hide();
            }
        } else if (showFrame && !playMode) {
            mFrame->show();
            mFrame->setBrush(QBrush(UBSettings::paletteColor));
            mFrame->setResizeHandlesVisible(true);
            updateButtons(true);
        } else if (!playMode) {
            // WistOpenboard fork: minimal-chrome path for "normal" items
            // (widgets, images, videos, etc.). Keep mFrame visible so its
            // border ring catches drag-to-move events (without it, web-widget
            // bodies eat every click and the user can't move them) and paint
            // it as a faint blue ring so the user can SEE where to grab. All
            // chrome buttons, the rotate handle, and every resize handle
            // except bottom-right are hidden. The user gets:
            //   • click body → interact with widget
            //   • click visible thin border → drag to move
            //   • drag bottom-right handle → resize
            //   • click floating ✕ chip → delete
            mFrame->show();
            mFrame->setBrush(QBrush(QColor(80, 130, 200, 90)));
            mFrame->setResizeHandlesVisible(false);
            updateButtons(true);
            foreach (DelegateButton *btn, mButtons)
                btn->hide();
            // Re-enable just the bottom-right resize grip so the user can
            // resize without the rest of the chrome.
            if (frame()) frame()->showSingleBottomRightResizeHandle();
        } else {
            if (mFrame) mFrame->hide();
            updateButtons(false);
        }

        if (!isTextItem && showFrame && mToolBarItem && mToolBarItem->isVisibleOnBoard())
        {
            mToolBarItem->positionHandles();
            mToolBarItem->update();
            mToolBarItem->show();
        }
        else if (mToolBarItem)
        {
            mToolBarItem->hide();
        }

        // WistOpenboard fork: position the floating delete chip just above
        // the top-left corner of the item. Independent of mFrame so it works
        // even when the frame is hidden.
        if (mFloatingDeleteButton && !showFrame
            && UBStylusTool::Play != UBDrawingController::drawingController()->stylusTool())
        {
            QGraphicsScene *sc = mDelegated->scene();
            if (sc && mFloatingDeleteButton->scene() != sc) {
                if (mFloatingDeleteButton->scene())
                    mFloatingDeleteButton->scene()->removeItem(mFloatingDeleteButton);
                sc->addItem(mFloatingDeleteButton);
            }

            // Cosmetic constant size regardless of zoom — scaled down to a
            // small chip so it doesn't overpower small items.
            constexpr qreal kChipScale = 0.45;
            const qreal scale = mAntiScaleRatio * kChipScale;
            QSizeF iconSize = mFloatingDeleteButton->renderer()->viewBox().size();
            qreal w = iconSize.width()  * scale;
            qreal h = iconSize.height() * scale;

            QTransform tr;
            tr.scale(scale, scale);
            mFloatingDeleteButton->setParentItem(nullptr);
            mFloatingDeleteButton->setTransform(tr);

            // Position just above-left of the item's scene bounding rect.
            QRectF sbr = mDelegated->sceneBoundingRect();
            mFloatingDeleteButton->setPos(sbr.left() - w * 0.3, sbr.top() - h * 1.1);
            mFloatingDeleteButton->show();
        }
        else if (mFloatingDeleteButton)
        {
            mFloatingDeleteButton->hide();
        }
    } else {
