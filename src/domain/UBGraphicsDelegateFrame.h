...
class UBGraphicsDelegateFrame: public QGraphicsRectItem, public QObject
{
    public:
        ...
        // WistOpenboard fork: hide all resize / rotate handles. Used for the
        // minimal-chrome text item delegate (we keep mFrame visible only so
        // its TitleBarArea formatting buttons show, but suppress everything
        // else so the box around the item disappears).
        void setResizeHandlesVisible(bool visible);
        // Show only the bottom-right resize grip (with the rotate button and
        // every other resize handle hidden). Used for the minimal-chrome path
        // on widget / image / video items.
        void showSingleBottomRightResizeHandle();
