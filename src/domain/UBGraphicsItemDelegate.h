...
class UBGraphicsItemDelegate : public QObject
{
    ...
    protected:
        ...
        // WistOpenboard fork: standalone floating delete chip shown above the
        // top-left corner of any selected item — replaces the full delegate
        // frame for simple delete-only access.
        DelegateButton* mFloatingDeleteButton{nullptr};

        // WistOpenboard fork: track hover state so positionHandles() can show
        // the thin drag-border on hover (not only on click-selection).
        bool mHovered{false};
