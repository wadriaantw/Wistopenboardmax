...
void ub_message_output(QtMsgType type, const QMessageLogContext& context, const QString& msg) {
    ...
    // WistOpenboard fork: always write the log file (was gated on -verbose).
    // Diagnostics for the new features (snap-to-shape, lasso, desktop drop) are
    // hard to triage otherwise, since release builds suppress qDebug stderr.
    {
