    ...
    , mIsDesktopMode(false)
{
    ...
    connect(UBApplication::mainWindow->actionLasso, SIGNAL(triggered(bool)), this, SLOT(lassoToolSelected(bool))); // WistOpenboard fork

    // WistOpenboard fork: snap-to-shape is a toggle that writes through to UBSettings.
    UBApplication::mainWindow->actionSnapToShape->setChecked(
        UBSettings::settings()->boardSnapToShape->get().toBool());
    connect(UBApplication::mainWindow->actionSnapToShape, &QAction::toggled,
            this, [](bool on) {
                qDebug() << "[snap-to-shape] toggle ->" << on;
                UBSettings::settings()->boardSnapToShape->set(on);
                UBApplication::showMessage(
                    QString("Snap to shape: %1").arg(on ? QStringLiteral("ON") : QStringLiteral("OFF")));
            });
...
void UBDrawingController::setStylusTool(int tool)
{
    if (tool != mStylusTool)
    {
        ...
        else if (mStylusTool == UBStylusTool::Lasso) // WistOpenboard fork
            UBApplication::mainWindow->actionLasso->setChecked(true);
        ...
        if (mStylusTool == UBStylusTool::Selector || mStylusTool == UBStylusTool::Play || mStylusTool == UBStylusTool::Lasso)
        {
...
// WistOpenboard fork
void UBDrawingController::lassoToolSelected(bool checked)
{
    if (checked)
        setStylusTool(UBStylusTool::Lasso);
}
